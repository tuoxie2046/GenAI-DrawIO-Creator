...
MCP（Model Context Protocol）を介して、Claude Desktop、Cursor、VS CodeなどのAIエージェントでGenAI Drawio Creatorを使用できます。
...
[Releases ページ](https://github.com/genai-drawio-creator/genai-drawio-creator/releases)からお使いのプラットフォーム用のネイティブデスクトップアプリをダウンロードしてください：
...
サポートやお問い合わせについては、GitHubリポジトリでissueを開いてください。
